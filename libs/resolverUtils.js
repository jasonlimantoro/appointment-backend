...
export const filterToday = list => {
  ...
  const later = moment()
    .hours(23)
    .minutes(59);
  return list.filter(({ createdAt }) =>
    moment(createdAt).isBetween(midnight, later));
};
 ...
 * @returns {string}
 */
export const getLambdaRequestId = context =>
  getNestedObjectValue(context)(['event', 'requestContext', 'requestId']);
 ...
 * @returns {string}
 */
export const getAPIRequestId = context =>
  getNestedObjectValue(context)(['context', 'awsRequestId']);

export const constructFileName = ({ prefix = '', fileName, fileType } = {}) =>
  `${prefix}/${humanFormat(new Date())}-${fileName}.${mime.extension(
    fileType,
  )}`;

export const paginate = (res, itemToCursor) => {
  const edges = res.map(item => ({
    node: item,
